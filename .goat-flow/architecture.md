...
5. `internal/baseline` classifies JSON baseline entries one-to-one: exact rule/file/fingerprint pairs first, then remaining modern entries by contract-stable identity; legacy entries stay exact-only, duplicate cardinality is preserved, and unmatched prior rows are reported as resolved/stale. Hook baseline and git-base new-only filtering reuse the same matcher.
