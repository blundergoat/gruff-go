...
The analyzer runtime is CLI-only. `cmd/gruff-go/main.go` delegates to `internal/cli`, which dispatches `analyse`, `baseline`, and `list-rules`. `analyse` can auto-load `.gruff.yaml`, `.gruff.yml`, or `.gruff.json`, apply JSON baselines, filter findings by git changed lines, and render text, full JSON, summary JSON, SARIF, or GitHub annotation output. `baseline` writes a JSON baseline from current findings. `list-rules` returns registry metadata. There is no HTTP request path, application middleware, database layer, or service response flow in this checkout.
...
3. `internal/config` optionally loads strict gruff config from explicit `--config` or default `.gruff.yaml`, `.gruff.yml`, or `.gruff.json` files. The root `.gruff.yaml` mirrors the current default rule pack and leaves expansion rules disabled unless a future change intentionally opts in. The supported shape follows the gruff-family layout: `paths.ignore`, `allowlists.acceptedAbbreviations`, `allowlists.secretPreviews`, `selection.rules`, `selection.excludeRules`, `selection.pillars`, `selection.excludePillars`, and `rules.<id>.enabled/threshold/thresholds/severity/options`. JSON legacy keys remain supported.
4. `internal/rule` validates rule definitions, sorts rule metadata by ID, dispatches enabled per-unit rules before project-level rules, and sorts findings deterministically. The default-enabled registry contains file length, function length, cyclomatic complexity, package comment, and secret-like assignment rules. Opt-in rules currently cover package names with underscores, empty control-flow blocks, shell command execution, and skipped tests.
...
7. `internal/scoring` computes severity/confidence-weighted per-pillar and composite scores after suppression/filtering.
...
9. `internal/report` renders the report as compact text, full JSON, summary JSON, SARIF 2.1.0, or GitHub annotations.
