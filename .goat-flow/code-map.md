...
- `.gruff-go.yaml` = Dogfood scanner config layering project-preferred thresholds and severities on top of the 79-rule registry.
...
- `internal/rule/` = Rule metadata validation, deterministic registry, configured thresholds/enablement, per-unit dispatch, project-level dispatch, composite-finding dispatch, finding ordering, and the 79-rule catalogue (70 default-enabled, 9 opt-in).
