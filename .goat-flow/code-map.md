...
- `.gruff-go.yaml` = Dogfood scanner config layering project-preferred thresholds and severities on top of the 63-rule registry.
...
- `internal/rule/` = Rule metadata validation, deterministic registry, configured thresholds/enablement, per-unit dispatch, project-level dispatch, composite-finding dispatch, finding ordering, and the 63-rule catalogue (57 default-enabled, 6 opt-in after ADR-016).
