...
- **Default policy reversal: every shipped rule is now `defaultEnabled: true`.** ADR-002's narrow 5-rule pack is superseded by [ADR-007](.goat-flow/decisions/ADR-007-comprehensive-default-rule-pack.md). Adopters running `gruff-go` for the first time now get the full 25-rule catalogue out of the box; disable per rule via `rules.<id>.enabled: false`. Severity discipline keeps default `--min-severity medium` CI gates stable — most flipped rules are `low` severity (naming.*, test-quality.*, dead-code.empty-block, docs.*, size.parameter-count, design composites) and appear in reports without flipping exit code. Two thresholds also moved toward industry-mainstream defaults: `complexity.nesting-depth` `maxDepth` `4 → 5` (matches `nestif`); `size.parameter-count` `maxParameters` `5 → 8` (matches revive `argument-limit`). Fingerprints, baseline schema, exit-code semantics, JSON schema version, and rule IDs are unchanged.

### Added (continued)

- **`naming.negated-boolean`** rule (M24 Phase 5): flags boolean identifiers whose names begin with negation prefixes (`No`, `Not`, `Disable`, `Disallow`, `Without`, `Suppress`) followed by an uppercase letter. Default-enabled `low` severity, parser capability. Configurable `prefixes`, `allowList`, and `scope` (`exported` default, `locals`, `all`). Type-aware: only flags identifiers whose syntactic type is `bool`. The default `allowList` covers English words like `NoOp`, `Notify`, `NoCopy`.
- **`naming.misspelling`** rule (M24 Phase 6): flags identifiers, doc comments, and struct tags containing tokens from a conservative built-in dictionary of common programming misspellings (`recieve`, `seperate`, `lenght`, `occured`, `enviroment`, etc., ~40 entries). Default-enabled `low` severity, parser capability. Tokens are extracted with camelCase / snake_case / non-letter splitting and matched exactly. Configurable `extra map[string]string` for project-specific additions and `ignore []string` for proper nouns that look like misspellings.
- **`naming.package-stutter`** rule (M24 Phase 7): flags exported top-level types, non-method functions, and package-scope variables/constants whose lowercase form starts with their own package name (`rule.RuleRegistry`, `httpserver.HttpServerOptions`, `config.ConfigOptions`). Default-enabled `low` severity, parser capability, `ProjectRule` (needs package context). Catches both exact-match stutter (allowlisted by default for `Config`, `Finding` per Go community convention) and prefix-then-uppercase stutter. Plain word extensions (`type Rules` in `package rule`) do not fire. Method names are not checked. Configurable `allowStutter []string` for project-specific exemptions. Rule catalogue is now **28 rules**.
