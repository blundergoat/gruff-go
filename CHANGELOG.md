...
- **Scanner hot paths avoid avoidable work in large trees.** Discovery skips full gitignore pattern evaluation when no applicable ignore rules exist, and `size.function-length` skips its code-line token scan for functions whose raw span is already under threshold. The performance harness now points its config comparison at `.gruff-go.yaml` and uses real config selection for per-rule timings.
