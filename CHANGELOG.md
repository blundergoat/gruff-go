...
- `docs/rules.md` per-rule sections for `complexity.nesting-depth` and `complexity.npath` now report the correct default thresholds (`maxDepth: 5`, `maxComplexity: 1024`) instead of the stale `4` / `9000` values that contradicted both the summary table and the source-of-truth constants in `internal/rule/`.
- `docs/rules.md` `sensitive-data.stripe-key` row now escapes the `|` characters inside the `(sk\|pk\|rk)_live_…` example so strict GFM renderers do not split the table cell on the unescaped pipes.
- `scripts/publish-go-pkg.sh` release gate is now consistent across the create-tag and pre-existing-tag paths: `require_clean_worktree`, `require_main_branch`, and `require_upstream_current` always run (the `--allow-non-main` opt-out remains the only bypass), and an existing local tag that does not point at HEAD now hard-fails with a `git checkout $TAG` hint instead of silently skipping release checks while still pushing the tag. Closes the gap where a pre-created tag could publish from a feature branch or against a dirty worktree.
- `scripts/publish-go-pkg.sh` now refuses to publish v2+ tags when the module path in `go.mod` lacks the corresponding `/vN` suffix, surfacing the violation before the tag is pushed instead of letting `proxy.golang.org` reject it after the release ref already exists on the remote.
