...
- **Default exit-code thresholds split per command.** Gating commands now default to `advisory` (`analyse`, `summary`) — down from `medium` in the 5-bucket era — so default scans surface every applicable finding. Artifact-generation commands now default to `none` (`report`, `dashboard`), meaning they never exit 1 on findings since they exist for inspection, not CI gating. CI pipelines that previously failed only on `medium`+ should set `minimumSeverity.analyse: warning` in `.gruff-go.yaml` (or pass `--min-severity warning`) to retain the prior behaviour. Configure per-command defaults via the new `minimumSeverity:` block (see Added).
- **Analysis schema bumps `gruff-go.analysis.v0.1` → `gruff-go.analysis.v0.2`.** The keys in `Report.Summary.CountsBySeverity` and the `PillarDetail.{Critical,High,Medium,Low,Info}` fields change shape; consumers should re-pin against `v0.2`. Config schema stays at `gruff-go.config.v0.1` — the new `minimumSeverity:` block is additive and optional.
...
- **`analyse --format=html` Pillars table.** Renders the canonical 7-column Pillars table shared with the text and JSON summaries. Covers every applicable pillar sorted by findings descending then pillar ascending, so clean scans surface as grade A rows with score 100.00 and zero counts.
- **`penalty` field on the canonical `gruff.summary.v2` JSON pillar object** and analyse `pillarDetails` entries. Penalty is the raw, unclamped score deduction accumulated for the pillar (score is still `max(0, 100 - penalty)`), preserving worst-pillar ranking when scores floor at zero — e.g. a pillar with 200 advisory findings reports `penalty=200, score=0`, distinct from a pillar with 1 error finding reporting `penalty=30, score=70`. Text, markdown, and HTML pillar tables are unchanged.
- **`scripts/publish-go-pkg.sh`** publishes a tagged Go module release: verifies the tag against source metadata, warms `proxy.golang.org`, and checks a temporary `go install`. The release gate applies consistently across the create-tag and pre-existing-tag paths — `require_clean_worktree`, `require_main_branch`, and `require_upstream_current` always run (`--allow-non-main` opts out); an existing local tag that does not point at HEAD hard-fails with a `git checkout $TAG` hint instead of silently skipping checks; v2+ tags are rejected when `go.mod`'s module path lacks the corresponding `/vN` suffix.
- [ADR-009](.goat-flow/decisions/ADR-009-three-severity-model.md) captures the cross-port severity-model migration: rationale, mapping table, rejected alternatives, schema-bump and golden-regeneration plan.
- [ADR-010](.goat-flow/decisions/ADR-010-per-command-minimum-severity.md) captures the per-command minimumSeverity dimension: precedence rule, bump-deferral policy, and the cross-surface naming inconsistency (YAML `minimumSeverity:` key vs Go `FailThreshold` type).
...
- **HTML report's per-pillar section** now renders as a 7-column table replacing the previous card grid. Scores render with two decimal places to match the canonical Pillars block, and every applicable pillar is always shown (clean pillars surface as grade A rows with zero counts).
...
- **Dogfood `.gruff-go.yaml` per-rule overrides** migrate from the old severity vocabulary to the new one (`notice` → `advisory`, `critical` → `error`) as part of the 5→3 collapse; the dogfood configuration remains grade A under the new defaults.
...
- **`docs/rules.md` per-rule sections** for `complexity.nesting-depth` and `complexity.npath` now report the correct default thresholds (`maxDepth: 5`, `maxComplexity: 1024`) matching the summary table and source-of-truth constants in `internal/rule/`; the `sensitive-data.stripe-key` row now escapes the `|` characters inside the `(sk\|pk\|rk)_live_…` example so strict GFM renderers do not split the table cell.
