...
func TestRequestURLRecognizesTypedHTTPClient(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "typed client reassigned from factory",
			body: `var client http.Client
	client = newClient()`,
			want: 1,
		},
...
// TestRequestURLRecognizesInjectedHTTPClient keeps dependency-injected clients
// in the outbound sink set even though their declaration sits outside the body.
func TestRequestURLRecognizesInjectedHTTPClient(t *testing.T) {
	unit := parseOne(t, "handler.go", `package handler

import "net/http"

func fetch(client *http.Client, r *http.Request) {
	_, _ = client.Get(r.FormValue("url"))
}
`)
	findings := (RequestControlledURLRule{}).AnalyzeUnit(unit, Context{})
	if len(findings) != 1 {
		t.Fatalf("findings = %#v, want one injected HTTP-client finding", findings)
	}
}

// TestRequestURLHTTPClientAssignmentOrder classifies an interface receiver from
// the value it holds when each request runs, not from a later assignment.
func TestRequestURLHTTPClientAssignmentOrder(t *testing.T) {
	unit := parseOne(t, "handler.go", `package handler
...
func fetch(r *http.Request) {
	var client getter = localClient{}
	_, _ = client.Get(r.FormValue("local"))
	client = &http.Client{}
	_, _ = client.Get(r.FormValue("remote"))
}
`)
	findings := (RequestControlledURLRule{}).AnalyzeUnit(unit, Context{})
	if len(findings) != 1 {
		t.Fatalf("findings = %#v, want only the request made after assigning the HTTP client", findings)
	}
}

// TestRequestURLHTTPClientBranchState keeps possible HTTP-client paths visible
// while allowing an unconditional custom-client write to clear earlier state.
func TestRequestURLHTTPClientBranchState(t *testing.T) {
	testCases := []struct {
		name         string
		clientSetup  string
		wantFindings int
		...
		{
			name: "optional HTTP client assignment can reach request",
			clientSetup: `var client getter = localClient{}
	if r.Method == "POST" {
		client = &http.Client{}
	}`,
			wantFindings: 1,
		...
		{
			name: "unconditional custom assignment replaces HTTP client",
			clientSetup: `var client getter = &http.Client{}
	client = localClient{}`,
			wantFindings: 0,
		},
		{
			name: "custom assignment dominates request inside optional branch",
			clientSetup: `var client getter = &http.Client{}
	if r.Method == "POST" {
		client = localClient{}
		_, _ = client.Get(r.FormValue("branch-url"))
	}
	client = localClient{}`,
			wantFindings: 0,
		},
		{
			name: "mixed short declaration does not change existing static type",
			clientSetup: `var client getter = localClient{}
	client, marker := &http.Client{}, 1
	_ = marker
	client = localClient{}`,
			wantFindings: 0,
		},
...
func fetch(r *http.Request) {
	`+testCase.clientSetup+`
	_, _ = client.Get(r.FormValue("url"))
}
`)
			findings := (RequestControlledURLRule{}).AnalyzeUnit(unit, Context{})
			if len(findings) != testCase.wantFindings {
				t.Fatalf("findings = %#v, want %d", findings, testCase.wantFindings)
			}
		})
	}
}

// TestRequestURLParsedReceiverBindingRespectsLexicalScope prevents an inner
// fixed-value String method from inheriting an outer parsed URL's request taint.
func TestRequestURLParsedReceiverBindingRespectsLexicalScope(t *testing.T) {
	unit := parseOne(t, "handler.go", `package handler
...
func (localURL) String() string { return "https://fixed.example.com/status" }

func fetch(r *http.Request) {
	parsed, _ := url.Parse(r.FormValue("url"))
	{
		parsed := localURL{}
...
`)
	findings := (RequestControlledURLRule{}).AnalyzeUnit(unit, Context{})
	if len(findings) != 0 {
		t.Fatalf("findings = %#v, want no finding for the inner fixed-value receiver", findings)
	}
}
...
// TestOpenRedirectConstraintDominance keeps optional prefix checks and
// normalization loops from hiding a later open redirect outside their branch.
func TestOpenRedirectConstraintDominance(t *testing.T) {
	t.Run("committed prefix guard inside optional branch does not protect later redirect", func(t *testing.T) {
...
func TestOpenRedirectReviewRegressions(t *testing.T) {
	t.Run("redirect status on a shadowed writer does not activate Location", func(t *testing.T) {
		unit := parseOne(t, "handler.go", `package handler
...
func (localWriter) WriteHeader(int) {}
...
`)
		findings := (OpenRedirectRule{}).AnalyzeUnit(unit, Context{})
		if len(findings) != 0 {
			t.Fatalf("findings = %#v, want no redirect finding for a different writer binding", findings)
		}
	})
