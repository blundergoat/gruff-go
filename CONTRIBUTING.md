...
- Every added or changed rule needs paired actionability coverage: keep a genuine finding firing, then apply the exact edit named by `Definition().Remediation` and prove the finding clears. When a matcher is narrowed, retain explicit false-positive and keep-firing fixtures instead of weakening or deleting the rule.
