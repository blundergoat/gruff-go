...
Thanks for considering a contribution. `gruff-go` is pre-release and the surface is small enough that focused PRs land quickly. This page covers the dev loop, the test gates the project expects to stay green, and a few conventions worth knowing before you open a change.
...
| `internal/cli/` | CLI parser; subcommand dispatch for `analyse`, `baseline`, `list-rules`, `dashboard`. |
...
| `internal/config/` | Strict `.gruff.yaml` / `.json` loader. |
...
1. Read `internal/rule/builtin.go` and `internal/rule/expansion.go` for the existing patterns. Pick the file that matches whether the rule is default-enabled or opt-in.
2. Implement the rule type with `Definition() Definition` and either `AnalyzeUnit(unit parser.Unit, ctx Context)` or a project-level analyzer.
3. Register the rule in `Defaults()` so `list-rules` picks it up.
4. Add a fixture and a unit test in `internal/rule/*_test.go`.
5. Update [`docs/rules.md`](docs/rules.md) and `.gruff.yaml` (if the rule is default-enabled) so dogfood reflects the new policy.
6. Run `make check` and confirm the self-scan still exits clean.
...
- If you broke a schema or CLI flag, note it explicitly in the PR — the project has not yet committed to backwards compatibility but pre-release breaks should be intentional, not incidental.
